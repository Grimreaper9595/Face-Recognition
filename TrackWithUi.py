...
blob = Blob.from_string("Storage Blob")
...
def trackImages():
    ...
    blob = storage.child('face-c29e4/'+ fileName).put(fileName)
    data = {'name': "Date_"+date+"  Time_"+Hour+"-"+Minute+"-"+Second, 'url': "firebase storage API%2FAttendance%5CAttendance_"+date+"_"+Hour+"-"+Minute+".csv?alt=media&token="+blob['downloadTokens']}
    result = firebase.post('/face-c29e4',data)
    print(result)
...
lbl3 = tk.Label(window, text="DESIGN BY RUSHIL(GRIM)", width=80, fg="white", bg="black", font=('times', 15, ' bold'))
